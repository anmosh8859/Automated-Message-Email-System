...
import { faUserPlus, faUserMinus, faUserEdit, faEllipsisV } from '@fortawesome/free-solid-svg-icons';
import styles from "../../../css/super.module.css";
  ...
  return (
    <div className={styles.container}>
      <div className={styles.section1} >
        ...
        <MDBCard style={{marginTop:'75px' , borderRadius:'15px' , height:'200px'}}>
        <ul><MDBBtn outline floating style={{marginTop:'25px'}}>
            <FontAwesomeIcon icon ={faUserPlus}></FontAwesomeIcon>
            <span className={styles.isAddAdminClicked} onClick={handleAddAdminClick}>Add Admin/Member</span>
          </MDBBtn>
        </ul>

          <ul><MDBBtn outline floating>
            <FontAwesomeIcon icon ={faUserMinus}></FontAwesomeIcon>
            <span className={styles.isAddAdminClicked} onClick={handleAddAdminClick}>Remove Admin/Member</span>
          </MDBBtn>
        </ul>

        <ul><MDBBtn outline floating>
          <FontAwesomeIcon icon ={faUserEdit}></FontAwesomeIcon>
          <span className={styles.isAddAdminClicked} onClick={handleAddAdminClick}>Edit Admin/Member</span>
          </MDBBtn>
        </ul>
        </MDBCard>
      </div>

      <div className={`${styles.section2} ${isSection2Active ? 'active' : ''}`}>
        {isAddAdminClicked ? (
          <AdminRegister/>
        ) : (
          <>
      ...
      {isMessageBoxOpen && (
        <div className={styles.messageBox}>
          <span className={styles.closeIcon} onClick={closeMessageBox}>X</span>
          <h4>Reply</h4>
